...
description: >
  ...
  decoding, KV cache, quantization, ngram, or a specific engine.
---
...
1. **Primary sources first.** Official docs, llama.cpp source and `--help`,
   HF model cards + API (`/api/models/<org>/<repo>/tree/main` for exact file
   sizes, `gguf` metadata fields), vendor papers and blogs. Follow each claim
   to the source that owns it. Reference external sources by URL, never
   copy-paste.
2. **Ground on this rig.** A vendor claim is marketing-grade until measured.
   Before proposing anything, check the repo's own measurement history
   (`results.tsv`, `docs/sessions/`, `docs/discovery/`) — a measured dead end
   stays dead unless the Fingerprint or engine changed. Probe flag support
   against the pinned engine (`scripts/setup-check.sh` reads `--help`). Use
   venv Python + harness modules for file facts (`gguf_has_mtp`,
   `gguf_kv_f16_mb`, `is_moe_model`, `resolve_model_file`) — never guess arch
   or size from filenames.
3. **Land every finding in one bucket:**
...
4. **Deliverables.** Durable technique facts → `docs/discovery/` (stable
   contracts only). Single-day capture →
   ...
   new work.
5. **Constraints.** Investigation is read-only: no Trials, downloads, or
