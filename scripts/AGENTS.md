...
- `rank_results.py` to print Pareto / Day / Night / claw / coding / agentic-coding rankings from `results.tsv` (Point = GGUF basename, ADR 0012 / Night `agentic_coding` ADR 0013). Agents must use this CLI for model rankings — no ad-hoc temp filter scripts.
...
- Use `rebuild_results_db.py` to force-rebuild the derived `results.db` SQLite mirror from `results.tsv` and verify parity (`trial_id` set + duplicate detection). Exit 0 on parity, 1 on drift. The mirror is refreshed automatically after every Trial write; this script covers manual rebuilds after hand-patching the TSV.
...
- [rank_results.py](rank_results.py) — Pareto / Day / Night / claw / coding / agentic-coding ranking over `results.tsv` (ADR 0006/0009/0012/0013).
...
- [rebuild_results_db.py](rebuild_results_db.py) — force-rebuild derived `results.db` mirror + parity check (`autoresearch/core/results_db.py` owns the logic).
