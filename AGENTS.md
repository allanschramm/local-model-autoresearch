...
- **No operator hardware fingerprint (hard)**: Tracked docs must not name the operator GPU SKU, hostname, exact MiB totals from `nvidia-smi`, personal paths, or identity. Say **discrete 8 GB-class / unified_memory / operator host / Baseline `VRAM_LIMIT_MB`**. Scores stay; hardware identity does not.
...
- **Thinking-model Claw scores (hard)**: Pre-fix Claw on hybrid-thinking GGUFs (`reasoning_content` / `enable_thinking` / card reasoning-on) may be false lows. Remasure Claw-full for those families after the agentic harness fix; do not remasure the whole library or coding-10 for this bug alone. Contract: [docs/discovery/thinking-models-claw-harness.md](docs/discovery/thinking-models-claw-harness.md).
...
- **Agent harness configs are machine-local**: `.agents/`, `.claude/`, `.cursor/`, `.pi/`, `.gemini/`, `docs/agents/`, and `skills-lock.json` are gitignored — skills, hooks, and harness wiring stay on the operator machine, not the clone. Hard-gates still apply where present locally. Do **not** require OS ACL (`icacls`), chmod lockdowns, or enterprise managed hooks for normal users.
...
- **Dense = no shared-memory offload**: Never partially offload dense GGUFs (layers to CPU / Windows shared GPU memory). That path freezes the whole PC. Only MoE may use expert offload (`--n-cpu-moe`). Dense must fit in **physical** VRAM on discrete GPUs, or in the **unified RAM pool** (with OS headroom) on Mac/UMA — cut `CTX_SIZE` / KV quant / drop draft or reject — never “spill and hope”. Dense free-at-start VRAM clamp (issue #10) stays on by default; operator escape `AUTORESEARCH_SKIP_FREE_CLAMP=1` uses configured `VRAM_LIMIT_MB` only (WDDM desktop false-rejects) — runtime monitor remains the kill guard.
- **NEVER Windows shared-GPU / pagefile spill (hard)**: Two failure modes: (1) dedicated near full → CUDA Sysmem Fallback — harness clamps ``physical − keepout`` and kills dedicated overshoot; (2) MoE+`NO_MMAP` parks host tensors in Task Manager **Shared GPU** (WDDM/PCI-e) while dedicated stays ~4–5 GB — Shared→pagefile/SSD freezes the PC. Kill absolute Shared over `SHARED_VRAM_LIMIT_MB` (default 2048). Set `GGML_CUDA_NO_PINNED=1` on cli/server. Keep `NO_MMAP` when it wins TPS.
