...
- **NEVER Windows shared-GPU / pagefile spill (hard)**: On discrete NVIDIA, never raise `VRAM_LIMIT_MB` above **physical VRAM**. True freeze = CUDA Sysmem Fallback when dedicated is near full (driver pages into Shared GPU / pagefile). **Not the same** as MoE+`NO_MMAP` experts in host RAM — Task Manager can show Shared while dedicated is fine; keep `NO_MMAP` when it wins TPS. Harness: Prefer No Sysmem Fallback (`ensure_no_sysmem_fallback` + `scripts/nvidia_disable_sysmem_fallback.nip`); clamp `VRAM_LIMIT_MB`; dedicated kill dense+MoE; Shared kill only when dedicated ≥ 90% ceil (or dense Shared alone). TPS `llama-cli` ctx-capped + watchdog. Unsafe escape: `AUTORESEARCH_ALLOW_SYSMEM_FALLBACK=1`.
