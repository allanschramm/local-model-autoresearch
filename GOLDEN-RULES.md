...
*   **Pre-flight Estimation**: `estimate_vram_mb` (weights + optional draft file + KV + overhead) runs before `llama-cli` / `llama-server`. Skip/reject any config with estimate `> VRAM_LIMIT_MB` (default 7900 on 8GB). Override via `ENGINE_DEFAULTS['VRAM_LIMIT_MB']` or `AUTORESEARCH_VRAM_LIMIT_MB`. `resolve_vram_limit_mb` hard-clamps to dedicated physical VRAM.
*   **Runtime NVML kill (dense + MoE)**: While `llama-server` runs, kills if dedicated `used_mb > min(VRAM_LIMIT_MB, physical_total)`. Shared-GPU kill only when dedicated ≥ 90% of that ceil (true CUDA sysmem spill) — MoE+`NO_MMAP` may show Shared with low dedicated (CPU experts; keep `NO_MMAP` for TPS).
*   **Prefer No Sysmem Fallback (hard)**: Before cli/server, `ensure_no_sysmem_fallback` applies NVIDIA DRS Prefer No Sysmem Fallback (`scripts/nvidia_disable_sysmem_fallback.nip`) so CUDA cannot page into Shared when VRAM fills. Escape: `AUTORESEARCH_ALLOW_SYSMEM_FALLBACK=1` (unsafe).
*   **TPS llama-cli guard**: Caps `-c` at `BENCH_CTX_CAP` (4096); watches dedicated (+ Shared only when armed).
