...
| `--cache-ram -1` `--kv-unified/--no-kv-unified` `--no-kv-offload` `--repack/--no-repack` `--op-offload/--no-op-offload` `:1704,2210` | `CACHE_REUSE 256` only, no `cache-ram`/`kv-unified`/`repack` | **Wire** — `cache-ram` caps KV MiB, `kv-unified` saves ~512MB @65k, `no-kv-offload` keeps KV on CPU for MoE (0.7GB q4_0 vs 2.6GB f16), `repack` +3% tg (see `cpu-inference-guide.md:6`) |
