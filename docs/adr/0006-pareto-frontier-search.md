...
3. **Baseline** is the Neighbor origin (active point), not the sole champion. Profile **Day** / **Night** selects that origin (manual override allowed): Day → max TPS *(superseded: [0007](0007-day-profile-speed-band.md) speed band, then [0008](0008-day-iq-epsilon-then-tps.md) IQ ε-band then max TPS)*; Night → among points with `CTX_SIZE ≥ NIGHT_CTX_FLOOR` (default 65536), max `min(agentic, coding)`; if none qualify, fallback to max ctx with a complete vector.
