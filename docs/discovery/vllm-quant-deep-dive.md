...
- **TBD:** When/if bitsandbytes actually migrates to an OOT plugin per RFC #39583 — tracked in that issue; bnb is in-tree as of 2026-08-02.
- **TBD:** Whether docs.vllm.ai's published hardware matrix catches up with the CUTLASS W4A8 (SM90) path; matrix row currently says Arm CPU only (stale vs source).
- **TBD:** GGUF plugin kernel coverage for `IQ2_M`/`IQ3_M` file-level types (no dedicated Triton dequant module; file-type acceptance exists but execution path is tensor-type driven).
