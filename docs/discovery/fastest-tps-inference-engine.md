# Fastest-TPS Inference Engine on This Rig — Research Guide

> Research note. Compiled 2026-08-02. Rig: **RTX 4060 8 GB VRAM, Ryzen 7 5800X (8C/16T), 32 GB RAM, Windows 11** (per `scripts/check_hardware.py`). Question: which inference engine maximizes single-user generation TPS on this machine. Claims cite owning primary source; cross-engine TPS claims that are not measured on this rig are flagged as unverified.

## 1. Ground truth: what this rig already achieves

Measured llama.cpp (upstream, CUDA build, vendored submodule at tag `b10173`) numbers from `results.tsv` — the repo's only 4060-TPS ground truth (repo rule: TSV wins over docs/estimates):
...
Pattern: **small-active MoE + speculative MTP heads beat dense 9B by 2.5–4x** on this rig. Context is expensive: dense 9B at 100k ctx ≈ 42 TPS, at 32k ctx ≈ 64 TPS (KV cache + attention dominate).
...
| **ExLlamaV3 / EXL3** | ✅ (source build) | ❌ (EXL3 format) | ✅ (consumer target) | vendor claims memory-bound GEMM; 4060 numbers not published | **Only credible raw-TPS challenger** |
...
- **llama.cpp Vulkan**: portable backend; on NVIDIA, the CUDA backend is the optimized path (llama.cpp backend list: CUDA for NVIDIA, Vulkan generic) — no reason to switch on this rig.
- **OpenVINO**: Intel CPU/iGPU play; 5800X has no iGPU, CPU-only INT8 won't beat CUDA on a 4060 ([openvino-genai-cpu-igpu-guide.md](./openvino-genai-cpu-igpu-guide.md) covers the AMD/Intel-CPU story).
...
llama.cpp already owns this rig's speed levers (all repo-measured, `config.py`-only):
...
**Upstream llama.cpp CUDA remains the fastest *practical* TPS engine on this rig.** It holds the measured records (MoE 178.5, MTP +80%), natively supports Windows + the GGUF store, and all remaining speed lives in model/flag choice, not engine choice.

**ExLlamaV3 (EXL3)** is the only credible raw-TPS challenger that runs on Windows NVIDIA, but it targets the dense-short-context niche this rig already beats via MoE/MTP, breaks the GGUF store and harness contracts, and has **zero measured 4060 evidence** (web search, §7). Track it in the landscape; if a future need is "maximum single-stream TPS for one dense Qwen3.5-class model at ≤16k ctx," measure ExLlamaV3 vs llama.cpp on *this rig* before switching (repo rule: measure, never estimate; no eval-score floors).

## 7. Web-search addendum (external benchmarks)

Cross-engine TPS numbers found in web sources, with fidelity flags. None overturn the repo-measured llama.cpp baseline on this rig — and several are from old builds/formats.

### 7.1 RTX 4060 llama.cpp CUDA (lab data, primary)
...
| :-- | --: | --: |
| RTX 4060 | **15.11** | 272 |
| RTX 4060 Ti | 22.06 | 288 |
| RTX 3080 Ti | ~31 | >4070 SUPER |
| RTX 2080 Ti | ≈4060 (bandwidth-bound) | — |

Caveats vs this repo: b3140 predates years of CUDA kernel work (repo runs b10173); Phi-3-mini is dense 3.8B, not the small-active MoE/MTP class that produces this rig's records (LFM2.5-8B-A1B 178.5 t/s). Takeaways: (a) 4060 prompt processing is strong (4th-gen tensor cores, 272 t/s pp even on b3140); (b) generation on dense models ≈ bandwidth-bound — 2080 Ti ≈ 4060 — which is exactly why small-active MoE wins here.

### 7.2 Official NVIDIA reference (primary)

NVIDIA developer blog (2024-10-02): llama.cpp, Llama 3 8B, ~**150 t/s on RTX 4090** (seq 100/100) — [link](https://developer.nvidia.com/blog). Scaling reference: 4090 is ~3.3x the 4060's bandwidth/compute; dense-8B TPS on this rig (Qwythos-9B 41–42 @ 100k ctx) is consistent with that ratio.
...
- ExLlamaV3 vendor: Marlin-style GEMM memory-bound at 4bpw **on 4090**; Ampere efficiency "needs work"; **Ada (4060, sm89) unverified**.
...
**Net:** every ExLlama speed claim is either EXL2-era (archived engine), 4090/3090-class, or unverified on Ada. No web source shows ExLlamaV3 beating modern llama.cpp on a 4060 8 GB with big context. The only way to know would be an on-rig measurement; per harness contract that means a config.py Baseline experiment through the search loop (llama.cpp) plus a manual ExLlamaV3 run outside it — not requested, so documented here instead.

## 8. Sources

- Rig: `scripts/check_hardware.py` output, 2026-08-02 (RTX 4060 8 GB, Ryzen 7 5800X, 32 GB RAM, win32)
...
- Puget Systems 4060 llama.cpp lab bench (b3140, Phi-3-mini Q4): https://www.pugetsystems.com/labs/articles/llm-inference-consumer-gpu-performance/
