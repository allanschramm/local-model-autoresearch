...
- **Effort tiers (minimal / low / medium / high / xhigh / max) are template-gated, not universally expressible.** The pinned b10549 has `--reasoning-effort LEVEL` (`arg.cpp:3717`; wired 2026-08-29 as config.py-only Baseline key `REASONING_EFFORT` — see [reasoning-levels-mapping.md](./reasoning-levels-mapping.md)), and it binds the `reasoning_effort` template kwarg; only templates that READ the variable react (DeepSeek-V4, gpt-oss class, Qwen3.8-27B). The qwen35 arch template (Ornith, Qwen3.5/3.6, qwen35moe, and the Qwen3.8-4B distill) does **not** read `reasoning_effort`; the Qwen3.8-**27B** open-source template is the verified exception — ladder xhigh/medium/low, default xhigh (2026-08-23; re-verified from the embedded GGUF template 2026-08-29).
...
**Operator UX — curbing overthinking (verified 2026-08-25):** measured on Ornith-1.5-9B operator benchmark sessions (2026-08-20/21): a 713-assistant-turn session is ~90 % thinking by volume — think blocks mean 3.4k / median 1.6k / p90 8.7k / max 17.9k chars (answer+tool text mean 406 chars); one 2026-08-20 session contained a single 146,893-char think (~35k tokens, ~13 min at 44 t/s). **The long thinks do not self-repeat** (0 repeated 8-grams in the longest block) — this is RL-trained verbose deliberation, not a repetition loop. Vendor eval protocol runs agentic/coding with `max_new_tokens` up to 131k and 256k ctx — unbounded thinking was rewarded during training. Consequences:

- **`--reasoning-budget N` is the loop lever and it is server-side, template-independent** (vendored `common/reasoning-budget.cpp`: IDLE→COUNTING→FORCING→DONE; on exhaustion it *forces the end-of-thinking tag*, not plain truncation). `--reasoning-budget-message` injects a transition nudge before the forced end tag — forced cutoff without a nudge degrades quality, the nudge smooths the handoff to answer/tool-call. OpenAI API accepts per-request `reasoning_budget_tokens` / `thinking_budget_tokens` (server-common.cpp) — clients can override per task without a restart. `reasoning_control: true` additionally arms early end-of-reasoning: `POST /v1/chat/completions/control` `{"action":"reasoning_end"}` cuts a live think mid-flight (same server-side machinery). See [speculative-and-multi-gpu-tuning.md](./speculative-and-multi-gpu-tuning.md) for the full engine-lever map (verified 2026-08-23, upstream-anchored) — this section only adds the operator-UX measurement layer. Wall clock at 44 t/s (9B): budget 4096 ≈ 93 s think/turn, 2048 ≈ 47 s, 1024 ≈ 23 s.
- **Penalty-window trap:** repeat/presence/frequency penalties all feed one windowed sampler — `--repeat-last-n`, **default 64 tokens** (`common/common.h` `penalty_last_n = 64`; `sampling.cpp` `llama_sampler_init_penalties`). OpenAI-API `presence_penalty` maps to the same window (server-schema.cpp). A card's `presence_penalty` / `repeat_penalty` are near-inert against multi-kilo-token thinks unless `--repeat-last-n` is raised (e.g. 2048+); measured zero repetition ⇒ even then expect modest gains.
- **DRY (`--dry-*`) is not forwarded by the harness** (7 sampler keys) and targets repeated sequences the measured thinks don't produce — skip.
- **KV-cache precision is not the loop lever.** q4_0→q8_0 KV changes fidelity, not think length; the qwen35 hybrid arch measures q4_0@131k ≥ q8_0@131k on this rig (2026-08-25 A/B), and the family tolerates q4_0 KV near-losslessly. On a dense 9B @131k, q8_0 KV adds ~2.1 GB (peak ~9.5 GB) — physically unloadable on 8 GB-class; only an MoE 35B (KV 1.3→2.6 GB @131k) could run it. Use KV precision for long-ctx *quality*, not loop time.
- **Recommended "active" profile (falsifiable):** `--reasoning-budget 2048` + `--reasoning-budget-message "Stop thinking and act now."` for interactive/daily use (per-request `reasoning_budget_tokens` for hard tasks); optional A/B arm `--repeat-last-n 2048 --presence-penalty 1.5` (card general-task value). `--reasoning off` nukes the reasoning capability — last resort. System-prompt steering ("think briefly, then execute the next step") is free and orthogonal. **TBD:** think-token distribution + task pass rate at budget ∈ {1024, 2048, 4096} × {no message, message} on fixed tasks.
