...
- [`2026-08-24-codacus-cache-full-offload.md`](./2026-08-24-codacus-cache-full-offload.md) — literal creator config on `-MTP-GGUF` file (`-ncmoe 99 --no-mmap`, 48 slots): full ladder control 28.5 → cache 30.6 → MTP 32.4–34.8 → **stacked 41.5–42.1 t/s (+46 % decode)**, prefill −4 % combined; provenance trap documented (same basename, MTP vs non-MTP repos); false-crash lesson (watchdog killed the server, not a fork bug); still not token-identical.
