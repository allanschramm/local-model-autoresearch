...
test("Week 2 Day 1 readiness needs all three quizzes plus practice", () => {
  const progress = loadProgress({
    teach_quiz_pass_v1: JSON.stringify(["s2d1-q1", "s2d1-q2"]),
  });

  assert.equal(progress.isLessonReady("s2d1"), false);
  progress.markQuizPassed("s2d1-q3");
...
test("Week 2 Day 3 readiness needs all four quizzes plus practice", () => {
  const progress = loadProgress({
    teach_quiz_pass_v1: JSON.stringify(["s2d3-q1", "s2d3-q2", "s2d3-q3"]),
  });

  assert.equal(progress.isLessonReady("s2d3"), false);
  progress.markQuizPassed("s2d3-q4");
  progress.markPractice("s2d3", "simulated");
